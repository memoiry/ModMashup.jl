...
function parse_commandline()
    ...
    @add_arg_table s begin
        ...
        "--id"
            ...
            required = true
        "--labels"
            help = "For selection, it should be labels file name. For ranking, it should be query file name and we use it to label patients."
            arg_type = String
            default = "nothing"
        "--CV_query"
            help = "For selection, folder name where Query files stored. For ranking, single query file name for use to label patients"
            arg_type = String
        "--top_net"
...
function main()
    ...
    if parsed_args["command"] == "selection"
        ...
        # Txt file containing query internal id of each cross validaiton 
